...
//    private final DateRange dateRange;
    ...
    public void extractLineItems( TimeCard timeCard ) {
    	List<ConsultantTime>    list        = 
                timeCard.getBillableHoursForClient( client.getName() );
            Consultant consultant  = timeCard.getConsultant();
            
            list.stream()
                .filter( t -> t.getDate().getMonth() == invoiceMonth )
                .forEach( t -> addLineItem(
                    new InvoiceLineItem( 
                        t.getDate(),
                        consultant,
                        t.getSkill(),
    ...
    public String toReportString() {
        ...
        sb.append(paddingForReport("-", 10)).append("  ")
        .append(paddingForReport("-", 29)).append("  ")
        .append(paddingForReport("-", 18)).append("  ")
        .append(paddingForReport("-", 5)).append("  ")
        .append(paddingForReport("-", 10))
        .append("\n");
        int itemCount = 1;
        for (InvoiceLineItem lineItem : lineItems) {
