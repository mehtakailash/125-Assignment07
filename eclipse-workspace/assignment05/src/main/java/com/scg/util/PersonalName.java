...
public class PersonalName implements Comparable<PersonalName>, Serializable {
	...
	private static final long serialVersionUID = -8442814202447271924L;
...
//		personalName = lastName + "," + " " + firstName + " " + middle;
    ...
    @Override
    public int compareTo( PersonalName that )
    {
        int rcode;
        ...
        if ( this == that )
            rcode = 0;
        else if ((rcode = compareToLastName( that )) != 0 )
            ;
        else if ( (rcode = compareToFirstName( that )) != 0 )
            ;
        else
            rcode = compareToMiddleName( that );
        
        return rcode;
    }
    
    private int compareToLastName( PersonalName that )
    {
        int rcode = this.last.compareTo( that.last );
        
        return rcode;
    }
    
    private int compareToFirstName( PersonalName that )
    {
        int rcode = this.first.compareTo( that.first );
        
        return rcode;
    }
    
    private int compareToMiddleName( PersonalName that )
    {
        int rcode = this.middle.compareTo( that.middle );
